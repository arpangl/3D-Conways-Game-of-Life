...
void print_usage() {
    std::cout << "Usage:\n"
                  ...
                  << "  Common additional flags: --seed <uint64> --output_every <n>\n"
    ...
    try {
        if (dimension == "2d") {
            ...
            if (auto it = args.find("seed"); it != args.end()) {
                options.seed = std::stoull(it->second);
            }
            if (auto it = args.find("output_every"); it != args.end()) {
                options.output_every = std::stoull(it->second);
            }
            if (auto it = args.find("seed"); it != args.end()) {
                options.seed = std::stoull(it->second);
            }
            ...
            if (options.species > 1) {
                ...
                // Default to Custom/CSV if not specified, or just ignore.
            }
            if (auto it = args.find("cuda_device"); it != args.end()) {
                options.cuda_device = std::stoi(it->second);
            }
            if (auto it = args.find("cuda_streams"); it != args.end()) {
                ...
                if (auto it = args.find("density"); it != args.end()) {
                    options.initial_density = std::stod(it->second);
                }
            if (auto it = args.find("avx2_tile"); it != args.end()) {
                options.avx2_tile = std::stoull(it->second);
            }
