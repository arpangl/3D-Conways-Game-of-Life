...
std::vector<RuleGroup> parse_rule_string(const std::string& raw) {
...
void run_3d_avx2_multi_species(const SimulationOptions3D &options) {
    ...
    std::vector<uint8_t> neighbor_counts(S * pN, 0); // Stores neighbor counts (0-26)
    ...
    for (std::size_t iter = 0; iter < max_iter; ++iter) {
        ...
        for (int z = 1; z <= D; ++z) {
            for (int y = 1; y <= H; ++y) {
                ...
                for (int x = 0; x < W; ++x) {
                    ...
                    // std::vector<uint8_t> local_n_counts_vec(S);
                    // std::vector<uint8_t> current_alive_vec(S); // Use uint8_t instead of bool for data() access
                    ...
                    for (int s = 0; s < S; ++s) {
                        ...
                        if (wants_to_live) {
                            ...
                            if (sp.strength > max_strength) {
                                ...
                                // Tie-breaker: Incumbent wins, else Lowest Index
        ...
        if (!skip_output && (output_every > 0 && (iter % output_every == 0 || equal_all || iter + 1 == max_iter))) {
            // 1. Generate projection (存儲物種 ID，用於彩色輸出)
            std::vector<uint8_t> proj(W * H, 0);
            for(int z=1; z<=D; ++z) {
                for(int y=1; y<=H; ++y) {
                    for(int x=1; x<=W; ++x) {
                        size_t pidx = z * pH * pW + y * pW + x;
                        size_t proj_idx = (y-1) * W + (x-1);
            ...
            for (int zz = 1; zz <= D; ++zz) {
                ...
                for(int y=1; y<=H; ++y) {
                    for(int x=1; x<=W; ++x) {
                        size_t pidx = zz * pH * pW + y * pW + x;
                        size_t slice_idx = (y-1) * W + (x-1);
                        ...
                        slice[slice_idx] = best_id;
                    }
                }
                char spath[256];
                std::snprintf(spath, sizeof(spath), "%s/frame_%04zu_slice_%03zu.ppm", sdir.c_str(), iter, (size_t)zz-1);
    ...
    std::cout << "3D AVX2 multi-species (CSV rules) run time: " << dt.count() << " s" << std::endl;
}