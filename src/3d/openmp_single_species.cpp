...
void run_3d_openmp_single_species(const SimulationOptions3D &options) {
    ...
    // Use padded buffer + parallelize over Z slices to reduce boundary overhead and thread contention
    const std::size_t Wp = W + 2;
    const std::size_t Hp = H + 2;
    const std::size_t Dp = D + 2;
    const std::size_t NP = Wp * Hp * Dp;
    std::vector<uint8_t> curp(NP);

    auto idxp = [&](std::size_t x, std::size_t y, std::size_t z) -> std::size_t {
        return z * (Wp * Hp) + y * Wp + x;
    };

    auto t0 = std::chrono::steady_clock::now();
    for (std::size_t iter = 0; iter < max_iter; ++iter) {
        // fill padded buffer once per iteration
        for (std::size_t zp = 0; zp < Dp; ++zp) {
            long zsrc = (long)(zp) - 1;
            zsrc = (zsrc % (long)D + (long)D) % (long)D;
            for (std::size_t yp = 0; yp < Hp; ++yp) {
                long ysrc = (long)(yp) - 1;
                ysrc = (ysrc % (long)H + (long)H) % (long)H;
                for (std::size_t xp = 0; xp < Wp; ++xp) {
                    long xsrc = (long)(xp) - 1;
                    xsrc = (xsrc % (long)W + (long)W) % (long)W;
                    curp[idxp(xp, yp, zp)] = cur[idx3((std::size_t)xsrc, (std::size_t)ysrc, (std::size_t)zsrc, W, H)];
                }
            }
        }
                ...
                for (std::size_t x = 0; x < W; ++x) {
                    ...
                    for (int dz = -1; dz <= 1; ++dz) {
                        for (int dy = -1; dy <= 1; ++dy) {
                            std::size_t base = idxp(xp - 1, yp + dy, zp + dz);
                            live += curp[base + 0];
                            live += curp[base + 1];
                            live += curp[base + 2];
                        }
                    }
                    live -= curp[idxp(xp, yp, zp)];
                    uint8_t alive = curp[idxp(xp, yp, zp)];
                    uint8_t out = 0;
    ...
    std::cout << "3D OpenMP run time: " << dt.count() << " s" << std::endl;
}
