...
static void write_ppm_multi(const std::string &path, const std::vector<int> &grid, std::size_t width, std::size_t height, int species) {
    ...
    if (!f) return;
    std::fprintf(f, "P6\n%zu %zu\n255\n", width, height);
    // simple palette for species (0 = black)
...
void run_2d_cpu_multi_species(const SimulationOptions2D &options) {
    ...
    const std::size_t output_every = options.output_every > 0 ? options.output_every : 1; // per user request
    mkdir("frames", 0755);

    for (std::size_t iter=0; iter<max_iter; ++iter) {
        // For each cell, count neighbors per species
        for (std::size_t y=0;y<H;++y) {
            for (std::size_t x=0;x<W;++x) {
                ...
                int out = 0;
                if (me != 0) {
                    // survive if same-species neighbors == 2 or 3
                    if (counts[me] == 2 || counts[me] == 3) out = me;
                    else out = 0;
                } else {
                    // birth: any species with exactly 3 neighbors
                    int chosen = 0;
                    int best_count = -1;
                    for (int s=1; s<=S; ++s) {
                        if (counts[s] == 3) {
                            // choose by highest raw neighbor count among contenders (deterministic)
                            if (counts[s] > best_count) { best_count = counts[s]; chosen = s; }
                            else if (counts[s] == best_count && chosen != 0) {
                                // tie-breaker: smaller species id
