...
void run_2d_avx2_single_species(const SimulationOptions2D &options) {
    ...
    for (std::size_t iter = 0; iter < max_iter; ++iter) {
        // Use a simple loop structured to be auto-vectorized by the compiler.
        // We keep the attribute `target("avx2")` so the compiler can emit AVX2 code here,
        // but avoid fragile byte-level intrinsics that caused correctness issues.
        for (std::size_t y = 0; y < H; ++y) {
            std::size_t y_prev = (y + H - 1) % H;
            std::size_t y_next = (y + 1) % H;
            #pragma GCC ivdep
    ...
    std::cout << "AVX2 run time: " << dt.count() << " s" << std::endl;
}
