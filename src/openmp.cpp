...
void step_openmp(const Grid3D& current, Grid3D& next, const LifeConfig3D& cfg) {
    if (current.size() != cfg.width * cfg.height * cfg.depth) {
        throw std::invalid_argument("current grid size mismatch (OpenMP)");
    }
    if (next.size() != current.size()) {
        next.resize(current.size());
    }

#pragma omp parallel for collapse(3) if (cfg.width * cfg.height * cfg.depth > 2048)
    for (std::ptrdiff_t z = 0; z < static_cast<std::ptrdiff_t>(cfg.depth); ++z) {
        for (std::ptrdiff_t y = 0; y < static_cast<std::ptrdiff_t>(cfg.height); ++y) {
            for (std::ptrdiff_t x = 0; x < static_cast<std::ptrdiff_t>(cfg.width); ++x) {
                int neighbors = 0;
                for (int dz = -1; dz <= 1; ++dz) {
                    for (int dy = -1; dy <= 1; ++dy) {
                        for (int dx = -1; dx <= 1; ++dx) {
                            ...
                            std::size_t nz;
                            if (cfg.wrap) {
                                nx = wrap_index(static_cast<std::size_t>(x + dx), cfg.width);
                                ny = wrap_index(static_cast<std::size_t>(y + dy), cfg.height);
                                nz = wrap_index(static_cast<std::size_t>(z + dz), cfg.depth);
                            } else {
                                const auto sx = x + dx;
                                const auto sy = y + dy;
                                const auto sz = z + dz;
                ...
                const auto idx = index_3d(cfg, static_cast<std::size_t>(x), static_cast<std::size_t>(y), static_cast<std::size_t>(z));
