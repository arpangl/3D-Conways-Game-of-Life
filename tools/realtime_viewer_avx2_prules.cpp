...
// [SIMD] 使用 AVX2 針對同一條列上的多個 cell 做 rule 判斷
...
static inline __m256i eval_rule_groups_avx2(
    ...
    // 傳回：每個 byte 是 0x00 或 0xFF，代表這個 group OR 結果是否滿足
    return result;
}
...
// Step 2: Resolve rules & competition（整段替換）
#pragma omp parallel
{
    ...
    for (int z = start_z; z <= end_z; ++z) {
        for (int y = start_y; y <= end_y; ++y) {
            ...
            while (x <= end_x) {
                ...
                // 針對每個 species，用 AVX2 評估整個 block 的 rule
                for (int s = 0; s < S; ++s) {
                    const auto& sp = species_list[s];

                    // 取出目前該 species 在這個 block 上是否存活（0 或 1）
                    ...
                    for (int j = 0; j < 32; ++j) {
                        if (j < blockLen) {
                            ...
                            alive_buf[j] = v;
                            if (v) {
                                // 與原本一樣：若多種 species 存在，同一 cell 取最後一個 s 當 current_occupant
                    ...
                    // wants_to_live = (alive ? survive_mask : birth_mask)
                ...
                // 把最佳 species 寫回 next_multi，並更新 bounding box
                for (int j = 0; j < blockLen; ++j) {
